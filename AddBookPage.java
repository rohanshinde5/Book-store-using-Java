...
public class AddBookPage extends JFrame {
    ...
    private void addBook() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            try (Connection conn = DBConnection.getConnection()) {
                String insert = "INSERT INTO books (id, title, author, publisher, year, price, quantity) VALUES (?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement stmt = conn.prepareStatement(insert);
                stmt.setInt(1, Integer.parseInt(id.getText().trim()));
                stmt.setString(2, title.getText().trim());
                stmt.setString(3, author.getText().trim());
                stmt.setString(4, publisher.getText().trim());
                stmt.setInt(5, Integer.parseInt(year.getText().trim()));
                stmt.setDouble(6, Double.parseDouble(price.getText().trim()));
                stmt.setInt(7, Integer.parseInt(quantity.getText().trim()));
                stmt.executeUpdate();
                loadBooks();
    ...
    private void updateBook() {
        ...
        int originalId = (int) model.getValueAt(row, 0);
        ...
        JTextField idField = new JTextField(String.valueOf(originalId));
        ...
        if (result == JOptionPane.OK_OPTION) {
            try (Connection conn = DBConnection.getConnection()) {
                String update = "UPDATE books SET id=?, title=?, author=?, publisher=?, year=?, price=?, quantity=? WHERE id=?";
                ...
                stmt.setDouble(6, Double.parseDouble(priceField.getText().trim()));
                stmt.setInt(7, Integer.parseInt(quantityField.getText().trim()));
                stmt.setInt(8, originalId); // original ID used in WHERE clause
                stmt.executeUpdate();
                loadBooks();
